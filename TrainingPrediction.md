...
The data processing and analysis steps to train the PREPRINT classifier and to predict the genome-wide enhancers.
...
```
The resulting .RData object resides in `$path_to_dir/Data/K562/data_R/1000_enhancers_bin_100_window_2000.RData`

For the GM12878 cell line, the data needs to be normalized wrt. K562 data 
...
```
For the GM12878 cell line, data normalized wrt the K562 data 
```
cell_line=GM12878
DNaseFile=$path_to_dir/Data/GM12878/raw_data/wgEncodeOpenChromDnaseGm12878Pk.narrowPeak.gz
normalizeBool=TRUE
...
Generate the data for the whole genome using bin size 100 (resolution). The files generated by this step are needed to generate the random genomic locations. Example for chromosome 1 and chromatin feature Ctcf
...
```
The data will reside e.g. in folder
```
...
Process the whole genome data into an R object and normalize the data. Quite fast (30min) but requires a lot of memory. The following steps require 4 hours and 30 G of memory. 
...
This step has similar time and memory requirements as extracting the training or test data enhancers and promoters. p300 sites, TSS and ENCODE blacklists are removed from the random regions.
...
### Generate training data set from all K562 training examples and testdata for GM12878 
...
### Training of the classifier, cross validation within the training cell line 
...
### Train the classifier using the whole K562 data 
```
window=2000
bin_size=100
N=1000
...
### Test on the GM12878 data and use the trained model to predict enhancers in the whole genome in both cell lines, K562 and GM12878
```
window=2000
bin_size=100
N=1000
...
```
### Computation of the best operating points, AUC, FDR, TPR, FPR, and 1% FPR thresholds
...
Predicting enhancers genome-wide, this also creates the data in bigWig format in 100 bp bins to visualize in a genome browser. Requires 6 hours with 12 cpus and 20G mem per cpu. Creates also the training data files for SVM.
```
...
### Collect RFECS predictions, make bedfiles
